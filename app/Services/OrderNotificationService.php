...
class OrderNotificationService
{
    ...
    public function notifyVendorOrderAssigned(Order $order): void
    {
        ...
        foreach ($vendors as $vendor) {
            ...
            // Broadcasting events disabled for MVP - using polling instead of WebSockets
            // These events were causing 500 errors and not being used
            // Can be re-enabled later if WebSocket support is needed
    ...
    {
        try {
            $usersToNotify = User::whereIn('role', ['admin', 'delivery'])->get();
            foreach ($usersToNotify as $user) {
                $user->notify(new \App\Notifications\NewOrderPushNotification($order));
            }
        } catch (\Throwable $e) {
