...
<head>
    ...
    <title>Bari Rider — Iniciar Sesión</title>
...
<body>
    <div class="relative w-full h-full min-h-screen flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%);">
        ...
        <div class="glass-card">
            ...
            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Acceso Repartidores</h1>
            <p class="text-sm text-slate-500 mt-1">Inicia sesión para gestionar tus entregas</p>
        </div>
        ...
        <form method="POST" action="{{ route('login', absolute: false) }}">
            ...
            <div class="input-group" x-data="{ show: false }">
                ...
                <button type="button" class="show-password-btn" @click="show = !show">
                    <!-- Eye Icon -->
                    <svg x-show="!show" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    <!-- Eye Off Icon -->
                    <svg x-show="show" style="display: none;" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="flex items-center justify-between mb-6 text-sm">
                <label for="remember_me" class="inline-flex items-center text-slate-600 cursor-pointer">
                    <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500/30">
                    <span class="ms-2 font-medium">Recordarme</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-indigo-600 hover:text-indigo-700 font-semibold transition" href="{{ route('password.request', absolute: false) }}">
        ...
        <!-- Register Link -->
        <div class="mt-8 text-center text-sm border-t border-slate-200/60 pt-6">
            <span class="text-slate-500 font-medium">¿Aún no eres repartidor?</span>
            <a href="{{ route('register.delivery', absolute: false) }}" class="block mt-2 text-indigo-600 hover:text-indigo-700 font-bold transition">
                Regístrate y empieza a entregar
            </a>
        </div>
    </div>
    </div>
