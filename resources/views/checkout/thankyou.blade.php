...
<body class="bg-[#FDFDFC] antialiased flex flex-col min-h-screen">
    ...
    @php
        ...
        $bankTransferConfigured = !empty($bankTransfer['account_holder']) || !empty($bankTransfer['bank_name']) || !empty($bankTransfer['alias']) || !empty($bankTransfer['cbu']);
    @endphp
        ...
        <div class="max-w-2xl w-full bg-white p-10 rounded-3xl shadow-sm text-center">
            ...
            @if ($orderId)
                <div class="mb-6 rounded-2xl border border-gray-200 p-5 text-left">
                    <p class="text-sm text-gray-500 uppercase tracking-wide mb-2">Detalle del pedido #{{ $orderId }}</p>
                    @if (!empty($orderItems))
                        <div class="space-y-2 text-sm text-gray-700">
                            @foreach ($orderItems as $item)
                                <div class="rounded-xl border border-gray-100 px-3 py-2">
                                    <p class="font-semibold text-gray-900">{{ $item['name'] }}</p>
                                    <p class="mt-1">{{ $item['quantity'] }} x ${{ number_format($item['price'], 0, ',', '.') }} = ${{ number_format($item['subtotal'], 0, ',', '.') }}</p>
            ...
            @if ($paymentMethod === 'transferencia')
                <div class="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-8 text-left text-sm text-amber-950">
                    ...
                    @if ($orderId || !is_null($total))
                        <div class="mb-4 rounded-2xl border border-amber-200 bg-white px-4 py-4">
                            ...
                            <p class="mt-2 text-base font-semibold text-amber-950">Transferi el importe exacto de tu pedido.</p>
                            @if (!is_null($total))
                                <p class="mt-2 text-3xl font-black tracking-tight text-amber-950">${{ number_format($total, 0, ',', '.') }}</p>
                            @endif
                            @if ($orderId)
                                <p class="mt-2 text-sm text-amber-900">Referencia para identificar el pago: <strong>Pedido #{{ $orderId }}</strong>.</p>
                            @endif
                        </div>
                    @endif

                    @if ($bankTransferConfigured)
                        <p class="mb-3 text-sm text-amber-900">Usa cualquiera de estos datos bancarios para realizar la transferencia:</p>
                    @endif
                    ...
                    @if (!$bankTransferConfigured && empty($bankTransfer['notes']))
                        <p>Tu pedido quedó registrado, pero faltan cargar los datos bancarios para mostrar acá las instrucciones completas de transferencia.</p>
                    @endif
                ...
                @if (!empty($whatsAppUrl) && $orderId)
                    <div class="mt-8 mb-8">
                        <div class="mb-4 rounded-2xl border border-green-100 bg-green-50 px-4 py-4 text-left text-sm text-green-950">
                            ...
                            <p class="mt-1 text-green-900">El mensaje ya sale armado con tu pedido #{{ $orderId }} y el total para que solo adjuntes la captura o comprobante.</p>
                        </div>
                        <a href="{{ $whatsAppUrl }}"
                            target="_blank"
                        ...
                        <p class="mx-auto mt-3 max-w-xl text-xs leading-relaxed text-gray-600">La acreditacion del pago se revisa manualmente desde administracion despues de recibir el comprobante.</p>
                    </div>
                @elseif ($orderId)
                    <div class="mt-8 mb-8 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-left text-sm text-amber-950">
                        <p class="font-semibold">Paso 2</p>
                        <p class="mt-2">Cargá un numero en <strong>BANK_TRANSFER_WHATSAPP_NUMBER</strong> o <strong>SUPPORT_WHATSAPP_NUMBER</strong> para mostrar el boton que permite enviar el comprobante por WhatsApp.</p>
                    </div>
                @endif
