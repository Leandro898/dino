<x-front-layout title="¡Gracias por tu compra! - Marketplace Bariloche" bodyClass="bg-[#FDFDFC] antialiased flex flex-col min-h-screen">
    @php
        ...
        <div class="max-w-2xl w-full bg-white p-10 rounded-3xl shadow-sm text-center">
            ...
            @if ($orderId)
                <div class="mb-6 rounded-2xl border border-gray-200 p-5 text-left">
                    <p class="text-sm text-gray-500 uppercase tracking-wide mb-2">Resumen del pedido #{{ $orderId }}</p>
                    
                    <div class="mt-3 space-y-2 text-sm text-gray-700">
                        @if (!empty($customerAddress))
                            <p>
                                <strong>Dirección de entrega:</strong> {{ $customerAddress }}
                            </p>
                        @endif
                        @if ($total)
                            <p class="text-lg font-bold"><strong>Total a pagar:</strong> ${{ number_format($total, 0, ',', '.') }}</p>
                        @endif
                    </div>
                </div>
            @endif
                    ...
                    <div class="mt-8 mb-8">
                        ...
                        <div class="text-center">
                            <a href="{{ $whatsAppUrl }}" target="_blank" rel="noopener noreferrer"
                                class="inline-flex w-full md:w-auto md:px-12 justify-center items-center gap-3 rounded-xl bg-green-500 px-5 py-4 text-sm font-bold uppercase tracking-wide text-white hover:bg-green-600 shadow-lg hover:shadow-xl transition-all"
            ...
            @if (!empty($trackingUrl))
                <div class="text-center mb-3">
                    <a href="{{ $trackingUrl }}"
                        class="inline-flex w-full md:w-auto md:px-12 justify-center items-center gap-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 transition-all text-white py-4 font-bold uppercase text-sm shadow-lg hover:shadow-xl"
                        id="track-order-btn">
            ...
            <div class="mt-4 mb-20 md:mb-12 text-center pb-8">
                <a href="{{ route('home') }}"
                    class="inline-block w-full md:w-auto md:px-16 bg-black hover:bg-gradient-to-r hover:from-purple-600 hover:to-purple-700 transition-colors text-white py-4 rounded-xl font-bold uppercase text-sm">
                    Volver a la tienda
                </a>
            </div>
        </div>
